...
section('1. 氷術師 active15種・進化8種が揃っている');
{
  ok(frostActives.length === 15, `氷 active15種 (${frostActives.length})`);
  ok(frostEvos.length === 8, `氷 進化8種 (${frostEvos.length})`);
}
...
section('3. Lv80発射数対象は frost_shard / glacial_lance / icicle_volley のみ（projectile タグだけで自動適用しない）');
{
  const lv80 = frostActives.filter((s) => appliesLv80ProjectileCount(s)).map((s) => s.id).sort();
  ok(JSON.stringify(lv80) === JSON.stringify(['frost_shard', 'glacial_lance', 'icicle_volley']), `Lv80対象=${lv80.join(',')}`);
