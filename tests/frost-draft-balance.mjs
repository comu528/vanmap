// 氷術師 完成監査 3/12: レベルアップ抽選シミュレーション（M7-E §4〜§7・§9・§10）。Node.js 標準機能のみ。
...
// 実行: node tests/frost-draft-balance.mjs   /   HEAVY=1 node tests/frost-draft-balance.mjs
...
import { analyzeFrostWarnings, DEFAULT_FROST_THRESHOLDS } from '../src/systems/FrostBalanceWarnings.js';
import { DATA, FROST, EXPECTED, registeredIds, draftCatalog, jobPools, seedRange, runner } from './frost-audit-common.mjs';

const T = runner('氷術師 抽選バランス・シミュレーション（M7-E）');
...
// （production は forceClear で救済して戦闘へ戻す）。枠が広いほど起きないことを確認する。
...
section('3. 進化到達率（evolution-first・60 level-up）— 警告基準（自動合否ではない）');
const TH = DEFAULT_FROST_THRESHOLDS;
...
for (const slot of SLOTS) {
  ...
  const n1 = TH.evoAtLeast1BySlot[slot], n2 = TH.evoAtLeast2BySlot[slot], nm = TH.evoMeanBySlot[slot];
...
// 基準未達は警告（docs へ理由を記載する）。ただし「1つも進化できない周回が過半」は不具合として落とす。
for (const slot of SLOTS) ok(run('evolution-first', slot).evoZeroRate < 0.5, `slot${slot}: 進化0個の周回が過半にならない`);
ok(true, `基準未達 ${belowThreshold.length} 件（警告のみ）`);
if (belowThreshold.length) for (const b of belowThreshold) info('警告: ' + b);

// ===== 4. 全戦略で全 active が提示され、取得もされる（ハズレ候補/死に候補の検出）=====
section('4. active30・passive4 の提示率/取得率（提示0・取得0が無い）');
const offerTotals = {}, pickTotals = {}, passivePickTotals = {}, evoExecTotals = {};
for (const id of FROST.activeSkillPool) { offerTotals[id] = 0; pickTotals[id] = 0; }
for (const id of FROST.passiveSkillPool) passivePickTotals[id] = 0;
for (const id of FROST.evolutionPool) evoExecTotals[id] = 0;
for (const policy of M7E_POLICIES) for (const slot of SLOTS) {
  const r = run(policy, slot);
  for (const id of FROST.activeSkillPool) { offerTotals[id] += r.activeAppearRate[id] || 0; pickTotals[id] += r.activePickRate[id] || 0; }
  for (const id of FROST.passiveSkillPool) passivePickTotals[id] += r.passivePickRate[id] || 0;
  for (const id of FROST.evolutionPool) evoExecTotals[id] += r.evoExecutedRate[id] || 0;
}
for (const id of FROST.activeSkillPool) {
  ok(offerTotals[id] > 0, `${id}: 候補として提示される（提示0でない）`);
  ok(pickTotals[id] > 0, `${id}: いずれかの戦略で取得される（死に候補でない）`);
}
for (const id of FROST.passiveSkillPool) ok(passivePickTotals[id] > 0, `passive ${id}: 取得される`);
for (const id of FROST.evolutionPool) ok(evoExecTotals[id] > 0, `${id}: いずれかの戦略・枠数で実際に進化する（到達不能でない）`);
...
section('5. レアリティ提示率と帯内の偏り');
{
  ...
  const groups = {};
  for (const id of FROST.activeSkillPool) (groups[rarityOf[id]] = groups[rarityOf[id]] || []).push(r.activeAppearRate[id] || 0);
...
section('6. passive4 の取得偏りと slot 充足率');
{
  ...
  const vals = FROST.passiveSkillPool.map((id) => r.passivePickRate[id] || 0);
  const mn = Math.min(...vals), mx = Math.max(...vals);
  ok(mn > 0 && mx / mn <= TH.passiveSkewRatio, `passive 取得回数の最大/最小比 ${(mx / mn).toFixed(2)} ≤ ${TH.passiveSkewRatio}`);
  info(`passive Lv4 到達率: ${FROST.passiveSkillPool.map((id) => `${id}:${((r.passiveLv4Rate[id] || 0) * 100).toFixed(0)}%`).join(' ')}`);
...
// ===== 9. FrostBalanceWarnings が実データで期待どおり動く =====
section('9. FrostBalanceWarnings（FROST_* 警告生成）');
{
  const rarityOf = {}; for (const s of DATA.skills) rarityOf[s.id] = s.rarity;
  ...
  ok(critical.length === 0, `severity=high の FROST 警告 0 件（実際 ${critical.length}）`);
  ok(!warns.some((w) => w.code === 'FROST_CATALOG_MISMATCH'), 'FROST_CATALOG_MISMATCH なし');
  ok(!warns.some((w) => w.code === 'FROST_POOL_LEAK'), 'FROST_POOL_LEAK なし');
  ok(!warns.some((w) => w.code === 'FROST_EVOLUTION_UNREACHABLE'), 'FROST_EVOLUTION_UNREACHABLE なし');
  ok(!warns.some((w) => w.code === 'FROST_ACTIVE_ZERO_OFFER' || w.code === 'FROST_ACTIVE_ZERO_PICK'), '提示0/取得0 の active なし');
  for (const w of warns.filter((x) => x.severity !== 'high')) info(`${w.severity}: ${w.code} ${w.target} — ${w.detail}`);
  // 警告生成器そのものが機能することを、壊れた入力で確認する。
  const broken = analyzeFrostWarnings({ catalog: { activeCount: 29, passiveCount: 4, evolutionCount: 18, issues: [], lv80Targets: [] }, expected: EXPECTED });
  ok(broken.some((w) => w.code === 'FROST_CATALOG_MISMATCH'), '数が合わなければ FROST_CATALOG_MISMATCH を出す');
  ok(broken.some((w) => w.code === 'FROST_LV80_MISMATCH'), 'Lv80対象が合わなければ FROST_LV80_MISMATCH を出す');
}

// ===== 10. 戦略ごとのサマリ出力（docs 用）=====
section('10. 戦略×枠数サマリ（docs/frost-draft-analysis.md と同じ数値）');
for (const policy of M7E_POLICIES) {
  for (const slot of SLOTS) {
    const r = run(policy, slot);
