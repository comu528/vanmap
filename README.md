...
> ⚠️ **開発状況**: 現在 **Milestone 9-A.2**（3 ジョブ実ブラウザ長時間プレイテスト・最終バランス判定）まで実装済みです。
> M9-A.2 は**検証の Milestone**で、**新しい active / passive / 進化 / ジョブを 1 件も追加せず、
> バランス値も 1 件も変更していません**。実ブラウザ（Chromium + 静的配信・**必須依存の追加 0**）で
> **production の通常 draft を使った通し run を 3 ジョブ × 3 strategy = 9 run**、
> Result / 拠点 / 再 run、敗北 run、進化取得、save→reload→再開、
> **品質 4 段階の長時間比較**、周回途中の品質切替、
> **stress（敵 100 体 + 弾多数 + ボス + 2 倍速 + 10 分相当）**、実キー入力まで実施し、
> **console / pageerror / requestfailed は合計 0 件**でした。
> 判定は `docs/final-balance-verdict.md`（**production の不具合 1 件を発見・修正／balance 変更 0 件**・
> 超過した目安はすべて `role` / `profile` / `harness` に分類）。
> **「楽しいか・爽快か・難しすぎないか」は自動操作では判定していません**——
> 人間のプレイテストは `docs/human-playtest-gate.md` に **open な manual gate** として残しています。
> 自動テスト **12 スイート追加（全 237 通過）**・`validate-data` へ M9-A.2 ブロック追加（0 エラー 0 警告）・
> **save_version は v6 のまま**。詳細は `docs/browser-longrun-playtest.md` / `docs/browser-longrun-results.md`。
>
> （M9-A.1 まで）**Milestone 9-A.1**（横断 balance ハーネス完成・実ブラウザ検証ゲート）。
> M9-A.1 は**測定基盤の Milestone**で、**新しい active / passive / 進化 / ジョブを 1 件も追加せず、
> バランス値も 1 件も変更していません**。M9-A の横断ハーネスに残っていた
> 「火 / 氷の弾ダメージ・DoT・場・reactive が解決されない」制約を、
> **production の `BattleScene.prototype` / `SpawnManager` / `Boss` / `Projectile` プールを
> Node から直接駆動する完全ハーネス**（`tests/cross-job-harness.mjs`）で解消しました。
> 未解決の damage 経路は 0 になり、共通 battle profile（normal / elite / boss / survival / stimulus）で
> 3 ジョブの DPS・boss kill time・生存力を同一条件で比較できます（比較結果と warning 分類は
> `docs/cross-job-final-balance.md`。**分類が bug / balance の warning は 0 件**）。
> 品質不変性も完全ハーネスで再監査し、**4 品質 + 周回途中切替で gameplay trace が byte-identical** を維持。
> 実ブラウザ検証は自動化（この環境の既存ツールのみ・**必須依存の追加なし**）で
> 起動 / 3 ジョブ実プレイ / 4 品質 / 品質途中切替 / F8 / セーブ往復 / 相対パスを **console エラー 0** で確認し、
> 実施済み / 未確認（手入力の体感・長時間・高負荷ピーク）を `docs/browser-validation-gate.md` に分けて記録しました。
> 観測の dead key 2 件（`burningDamage` の発行元なし・`_damageTakenTotal` の加算なし）を最小修正。
> 自動テスト **15 スイート追加（全 225 通過）**・`validate-data` へ M9-A.1 ブロック追加（0 エラー 0 警告）・
> **save_version は v6 のまま**。詳細は `docs/cross-job-full-balance-harness.md`。
>
> （M9-A まで）**Milestone 9-A**（3 ジョブ横断・共通システム総合監査）。
...
> 固有機構（闘気 / コンボ / 撃破回復 / 不屈 / 防御 / 敵種別 / 移動）・cap と死にフィールド・
> telemetry / F8 / F9 / F10・性能・セーブ往復・決定性・バランス分布の
> **12 観点**で総点検し、**不備 8 件 + 死にフィールド 1 件を修正**しました。
> 主な修正は、48 スキルの `cdLeft` 復元を共通 `restoreCd()` で改ざん耐性化（保存破損の防止）、
> 陣の半径 / 反撃窓の回数・持続 / 旋風の残り時間の上限クランプ、
> **進化済みの基礎 active が空き枠へ再提示されていた抽選漏れ**の修正、
> `destroy()` 後もスキルが動き続けた残留の修正、決闘マーカーの寿命の一元化、
> そして `crimson_execution` が base より弱かった逆転の解消（`safetyCaps` 1 値のみ）です。
> **敵 HP / 攻撃力 / 経験値 / 難易度倍率 / 報酬 / スキルの damage・cooldown は 1 件も変えていません。**
> **抽選しきい値は 1 つも下げていません**（M8-C.1 の目標をそのまま満たしています）。
> **火の魔女・氷術師は候補列・ランタイム・セーブ・状態異常 RNG とも完全に不変**（SHA-256 で一致）・
> **save_version は v6 のまま**。実ブラウザでの確認は未実施。
> 詳細は `docs/warrior-completion-audit.md`・`docs/warrior-completion-matrix.md`・
> `docs/warrior-completion-balance.md`。
>
> （M8-E まで）**Milestone 8-E**（戦士スキル拡張 最終Wave）。
> M8-E では戦士の **active を 5 種追加して計 30 種**、**進化を 5 種追加して計 18 種**にし、
> **3 ジョブがそろって 30 / 4 / 18 の同規模**になりました
> （passive は 4 種のまま・Job Lv1〜100 も据え置き）。
> 追加した active は 貫穿突き / 一騎討ち / 修羅の構え / 震天踏破 / 刃返し、
> 進化は 神速貫陣 / 覇王討ち / 血染修羅 / 大陸震砕踏破 / 天鏡返し です。
> 直線の対象選択・決闘・構え・進軍・弾き返しという新しい機構を足しましたが、
> **いずれも新しい状態異常を作らず**、`WarriorCombatSystem` の中で `balance.json` の上限つきで動きます。
> **一騎討ちは相手に何も貼らず**、戦士本人の補正としてだけ効きます（同時 1 体・敵オブジェクトを保存しない）。
> **修羅の構えのリスクは軽減の小幅低下だけ**で、重装 / 闘気解放 / 不屈のいずれも無効化せず、
> 合計軽減が負になることもありません（闘気解放との合成上限つき）。
> **刃返しは完全無効化ではありません**。弾けるのは通常の敵弾だけで、ボスの予兆 / 光条 / 地形ハザード /
> DoT は弾けず、窓ごとの上限を超えた弾はそのまま通ります。反射弾は**世代 1 で止まり再反射しません**。
> **Job Lv80「打撃数 +1」の対象は 3 ジョブとも 6 種のまま**で、遠距離へ飛ぶ弾も 1 つも増えていません。
> active が 30 に増えたぶんの抽選の薄まりは、**M8-C.1 のしきい値を 1 つも下げずに**
> guidance へ data キーを 2 つ足すだけで吸収しました
> （`highRequirementSupportLevel` / `highRequirementSupportMultiplier`）。
> **火の魔女・氷術師は候補列 300 seed と 48 スキルのランタイムとも SHA-256 で完全一致**・
> **save_version は v6 のまま**。実ブラウザでの確認は未実施。
> 詳細は `docs/warrior-final-wave.md`・`docs/warrior-evolution-guidance.md`・`docs/skill-catalog.md`。
>
> （M8-D まで）**Milestone 8-D**（戦士スキル拡張 Wave2）。
> M8-D では戦士の active を 10 種追加して計 25 種、進化を 5 種追加して計 13 種にしました。
> 打ち上げ・前面防御・掴み / 投げ・戦旗の陣・低 HP スケーリングという機構を足しています。
> 詳細は `docs/warrior-wave2.md`。
>
> （M8-C.1 まで）**Milestone 8-C.1**（戦士 4 枠時の進化導線修正）。
> M8-C.1 は**抽選導線の修正のみ**の Milestone で、新しいスキル / passive / 進化 / ジョブや、
> スキル数値・闘気 / コンボ / 回復 / 体勢のバランス変更は**一切ありません**。
> M8-C で戦士の active が 5 → 15 になった結果、**active 枠 4 の進化到達率が 92.5% → 46.5%** へ落ちていました。
> 原因は「**進化元（基礎 active）側へ効く抽選補正が production に無かった**」ことです
> （既存の補助は「基礎を持っている → その補助を煽る」の一方向だけで、
> 補助が passive 中心の戦士では基礎 Lv8 が最後まで残っていました）。
> テストのしきい値を下げるのではなく、`data/skill-config.json` の `guidance` ブロックと
> `SkillDraftManager` の重み計算を足して**導線そのもの**を直しました。特定スキル名のハードコードはありません。
> 結果、枠 4 / 40 レベルアップで **62.5% → 89.5%**（平均 0.70 → 1.26・進化 0 個 37.5% → 10.5%）。
> 枠 6 / 枠 8 も改善し、**最頻進化のシェアと build の多様性は逆に良くなりました**（過剰誘導なし）。
> **火の魔女・氷術師は候補列・ランタイムとも SHA-256 で完全一致**・**save_version は v6 のまま**。
> 実ブラウザでの確認は未実施。詳細は `docs/warrior-evolution-guidance.md`・`docs/warrior-draft-analysis-wave1.md`。
>
> （M8-C まで）**Milestone 8-C**（戦士スキル拡張 Wave1）。
...
| **M8-C.1** | **戦士 4 枠時の進化導線修正**（新規コンテンツ・バランス変更なし）: M8-C で戦士の active が5→15になった結果、production の `SkillDraftManager` を使った 200 seed シミュレーションで **active 枠4 の「進化1個以上」到達率が 92.5% → 46.5%** へ低下していた問題を修正。原因分析の結果、(a) 既存の synergy 補助（M6-F）は**「所持している基礎 → その補助スキル」の一方向しか無く**、逆方向（補助が揃っている → 基礎を伸ばす）が存在しなかったこと、(b) 既存 pity は進化成立でしかリセットされず**基礎 Lv8 の詰まりを救えなかった**こと、(c) 既存の到達率テストが `_generate()` 直呼びで synergy / pity / reroll を通していなかったこと、の 3 点が判明（進化 0 の周回では**所持基礎の最高 Lv が 7.03**＝あと 1 段階で届いていなかった）。火/氷は進化の補助が active 中心なので基礎も自然に育つのに対し、戦士は 8 進化中 7 件が passive 補助で passive が早期に最大化されるため、基礎 active の Lv8 だけが最後まで残る構造だった。修正は `data/skill-config.json` へ**ジョブ限定の `guidance` ブロック**を追加し、`SkillDraftManager._guidanceMult()` が **data の進化レシピからのみ**補正対象を導出する形（**特定 skill ID のハードコードなし**）。「どの進化元を取るか」はレアリティ階層に委ねたまま、「取った進化元を最後まで伸ばす」側だけを助ける（所持基礎の強化・最大Lv接近・補助が揃った基礎・未達補助・必要Lv手前の補助）。同じスキルが複数レシピに関わっても倍率は掛け算にせず役割ごとの最大値だけを採り（重複上限）、合成は `maxMultiplier` で必ず頭打ち。進化導線用の pity（`guidanceStall`）を追加し、進展（基礎の取得/強化・補助の取得/強化・進化取得）でリセット・**保存するので save/reload で稼げない**・**RNG 消費は 1 も増やさない**（重みだけを変える）。結果は枠4/40レベルアップで **62.5% → 89.5%**（平均 0.70 → 1.26・進化0個 37.5% → 10.5%）、枠6/60 で 99.0% → 100.0%（2個以上 70.5% → 91.0%）、枠8/80 で平均 3.50 → 3.92。**過剰誘導なし**（最頻進化シェア 34.8% → 30.4%・build 多様性 61.9% → 66.5%・新規取得率が同 rarity 中央値の3倍超は 0 件・進化を持たない active の取得は +3.9%）。M8-C で一時的に下げたテスト閾値を復元（枠4 ≥80%/平均1.0/0個≤20%、枠6 ≥95%/2個以上≥60%/平均1.7、枠8 ≥95%/2個以上≥65%/平均1.8）。自動テスト9スイート追加（**全125スイート通過**）。**火の魔女・氷術師は候補列300seedのSHA-256・48スキルのランタイムトレースとも完全一致**・**data のスキル/進化/バランスは1件も変更なし**・**save_version v6 維持**（draft へ `guidanceStall` を1キー加算的に追加）。実ブラウザ確認は未実施。詳細は `docs/warrior-evolution-guidance.md`・`docs/warrior-draft-analysis-wave1.md` | ✅ 実装済み |
...
| **M9-A.2** | **3 ジョブ実ブラウザ長時間プレイテスト・最終バランス判定**（**新コンテンツ 0 件・バランス値の変更 0 件**）: M9-A.1 の browser gate は「起動 / 3 ジョブ各 20 秒 / 4 品質の上限一致 / F8 / セーブ往復 / 相対パス」までだった。M9-A.2 は**通常 draft の通し run**（production の `SkillDraftManager` / `LevelUpScene` / `EvolutionScene` を通す。固定完成 build の代用ではない）を **3 ジョブ × 3 strategy（evolution-first / balanced / random-valid）= 9 run**、reroll / banish / skip も実仕様で使用。**Result / 残り火 / Job XP / 拠点 / 再 run**（同じ `resultId` の再処理で二重加算しないことを実機確認）、**敗北 run**（`active_run` 消去・少量報酬・再 run 可能）、**進化の実取得**（各ジョブ 3 種以上・進化後の base 再提示 0 / base と evolution の同時所持 0 / reload で二重にならない・通常 draft 到達と条件を組んだ取得を区別して記録）、**save → reload → つづきから**（jobId / Lv / XP / HP / active・passive・進化 / runtime・cooldown / RNG cursor / 状態 / draft の reroll・banish・skip・stall / 資源が一致。**無料 cast なし・cooldown 全回復なし・報酬二重なし**・`selectedJobId` を変えても `active_run.jobId` 不変・export/import 往復）、**品質 4 段階の長時間比較**（固定 build・同 seed。**gameplay 上限は 4 品質で完全同一**（敵 200 / 弾 400 / hitStop 有）・**取得スキルの集合も完全一致**・品質差は `particleBudget` と `damageNumbers` のみ）、**周回途中の品質切替**（low↔ultra / medium→high で飛行中の弾・敵・状態・上限が不変）、**stress**（敵 100 体維持 + 弾多数 + ボス + エリート + active30 + 進化 + 2 倍速 + **10 分相当** × low/high）、**実キー入力**（WASD / SPACE ダッシュ / Q オート切替 / ESC ポーズ）、F8 / F9 / F10 の構造確認まで実施。**console / pageerror / requestfailed は全フェーズ合計 0 件**。**計測は直列実行**（software GL で 3 並列にすると 27〜35fps まで落ち、2 倍速の論理 dt が 60〜75ms へ伸びて到達点が半減する。直列では平均 59fps ＝ 論理 dt ≒ 33ms で Node ハーネスの 32ms と同水準）。**実ブラウザでは byte-identical を主張しない**（`delta` が実時間のため。byte-identical の証明は固定 dt の Node ハーネスの担当で、テストが「主張していないこと」を機械検査する）。最終判定（`docs/final-balance-verdict.md`）は **production の不具合 1 件（発見・修正済み）/ balance 変更 0 件**で、超過した目安（DPS 比・生存差・top share・被弾比・回復比）はすべて **`role`（役割差）/ `profile`（build と strategy の偏り）/ `harness`（測り方の限界）**に分類。**「楽しいか・爽快か・難しすぎないか・氷のボス戦が退屈か・戦士が安全すぎないか・火が強すぎるか」は自動操作では一切判定せず**、`docs/human-playtest-gate.md` の **open な manual gate** に残した。ブラウザ自動化はリポジトリ外スクリプトで行い、**要約 JSON（`tests/browser-results/m9a2-results.json`）だけをコミット**（生ログ / スクリーンショットはコミットしない・`package.json` / Playwright 設定は追加しない）。自動テスト **12 スイート追加**（browser-longrun-gate / normal-draft-plan / evolution-plan / result-cycle / resume-plan / quality-plan / quality-midrun-plan / stress-plan / balance-verdict / human-feel-gate / cross-job-browser-node-consistency / three-job-final-playtest-nonregression・**CI はブラウザを起動せず計画 / ゲート / 記録 schema / Node との整合を検証**）・validate.yml へ 12 ステップ・validate-data へ M9-A.2 ブロック（**0 エラー 0 警告**）。**save_version v6 維持**。詳細は `docs/browser-longrun-playtest.md`・`docs/browser-longrun-results.md`・`docs/final-balance-verdict.md`・`docs/human-playtest-gate.md` | ✅ 実装済み |
| **M9-A.1** | **横断 balance ハーネス完成・実ブラウザ検証ゲート**（**新コンテンツ 0 件・バランス値の変更 0 件**）: M9-A の横断ハーネスに残った「火 / 氷の弾ダメージ・DoT・場・reactive が Scene モックでは解決されない」制約を解消。`tests/phaser-stub.mjs`（Phaser プリミティブの最小 stub）＋ `tests/cross-job-harness.mjs` が **production の `BattleScene.prototype` / `SkillManager` / 全スキル class / `Projectile`・`Enemy`・`Boss`・`ExperienceGem` / `SpawnManager` / `StatusEffectManager` / `WarriorCombatSystem` / `CombatTelemetry` を Node から直接駆動**し、置き換えるのは表示 / 保存 / シーン遷移だけ（`scene[m] === BattleScene.prototype[m]` を機械検査）。**独自 damage 式・即着弾扱い・DoT 一括化・回数推測・Math.random・品質分岐は一切なし**。弾（貫通 / 帰還 / 周回 / ホーミング / 爆発 / 連鎖 / 跳弾 / 分裂 / 二次世代 / 反射）・DoT（炎上 tick / 起爆 / 共鳴 / 感染）・場 / 遅延・氷（冷気 / 凍結 / 粉砕 / 氷砕ゲージ）・戦士（体勢 / 反撃 / 弾き / 処刑 / 掴み）・death event / XP / level-up まで**未解決の damage 経路 0**。共通 battle profile 5 種（normal / elite / boss / survival / stimulus・seed / dt32ms / 湧き / ボス / 敵弾 timeline が 3 ジョブ完全同一）で **DPS・boss kill time（火 7.6s / 戦士 12.5s / 氷 22.2s 中央値）・生存力を同一条件で初めて絶対比較**。144 スキル全件に utility を確認（死にスキル 0）・reactive は共通刺激 timeline（17 イベント）で全反応。warning はすべて **role / profile に分類**（bug / balance 分類 0 件＝バランス変更なし）。品質不変性を完全ハーネスで再監査: **4 品質 × 3 ジョブ × 全 profile + 周回途中切替（low→ultra→medium）で gameplay trace が byte-identical**。**実ブラウザ検証**は環境既存の自動化ツールのみで実施（**必須依存の追加 0**・package.json なしのまま）: Phaser 3.90.0 起動 / Title 到達 / 3 ジョブ実プレイ / 4 品質で gameplay 上限同値（敵 200 / 弾 400 / hitStop 有）/ 品質途中切替で周回継続 / F8 パネル / セーブ往復（v6）/ 相対パス（失敗 0・外部ホスト 0）を **console エラー合計 0** で確認。手入力の体感・長時間・高負荷ピークは**未確認**として `docs/browser-validation-gate.md` に区別して記録（再現手順 + 結果テンプレートつき）。観測だけの dead key 2 件を最小修正（`burningDamage` に発行元が無い / `_damageTakenTotal` に加算箇所が無い —— いずれも damage / RNG へ影響なし）。F8 分析パネルへ「横断バランス（M9-A.1）」節を追加（表示のみ）。自動テスト **15 スイート追加**（cross-job-full-harness / projectile-resolution / dot-persistent-resolution / reactive-stimulus / boss-profile / survival-profile / balance-final / balance-warning-classification / quality-full-invariance / quality-midrun-switch / balance-determinism / harness-save-isolation / harness-cleanup / browser-gate / three-job-balance-harness-nonregression・**全 225 スイート通過**・trace / 経路集合 / 測定条件を SHA-256 固定）・validate.yml へ 15 ステップ（計 226）・validate-data へ M9-A.1 ブロック（**0 エラー 0 警告**）。**save_version v6 維持**。詳細は `docs/cross-job-full-balance-harness.md`・`docs/cross-job-final-balance.md`・`docs/browser-validation-gate.md` | ✅ 実装済み |
...
## Milestone 8-C.1 の要素（戦士 4 枠時の進化導線修正）

M8-C で active が 5 → 15 に増えた結果、**active 枠 4（転生前）で進化まで届かない周回**が増えていました。
新しいスキルは足さず、**抽選の導線だけ**を直した回です。

### 何が起きていたか

進化 0 で終わった周回を実測すると、**所持している進化元の最高 Lv が平均 7.03**。
補助（passive）はほぼ揃っているのに、**基礎 active があと 1 段階だけ届いていません**でした。

既存の抽選補助は「基礎を持っている → その補助スキルを出やすくする」の一方向しか無く、
**基礎そのものを伸ばす方向の補助が存在しなかった**ことが主因です。
火 / 氷は進化の補助が active 中心なので補助を伸ばす過程で基礎も育ちますが、
戦士は 8 進化中 7 件が passive 補助で、passive は早期に最大化されるため基礎だけが残ります。

### どう直したか

「どの進化元を取るか」はレアリティ階層に委ねたまま、
**「取った進化元を最後まで伸ばす」側だけ**を助けるようにしました。

- 所持している進化元の強化を優遇する（最大 Lv に近いほど強く）
- 補助が揃っている進化元は、さらに優遇する
- 必要 Lv 未達の補助を優遇する（必要 Lv 手前でさらに）
- 導線が一定回数進まなければ段階的に強め、進んだら戻す（pity・保存するので再読込で稼げない）

補正値は `data/skill-config.json` に置き、対象は**進化レシピから自動で導出**します
（特定スキル名を実装に書いていません）。倍率には必ず上限があり、進化の強制付与もしません。

### 結果（200 seed・production の抽選器で実測）

| 構成 | 前 | 後 |
|------|----|----|
| 枠4 / 40 レベルアップ・進化 1 個以上 | 62.5% | **89.5%** |
| 枠4 / 40 レベルアップ・平均進化数 | 0.70 | **1.26** |
| 枠4 / 40 レベルアップ・進化 0 個 | 37.5% | **10.5%** |
| 枠6 / 60 レベルアップ・1 個以上 / 2 個以上 | 99.0% / 70.5% | **100.0% / 91.0%** |
| 枠8 / 80 レベルアップ・平均 | 3.50 | **3.92** |

**偏りは増えていません**。最頻進化のシェアは 34.8% → 30.4%、
build（所持 active の組み合わせ）の種類はむしろ 61.9% → 66.5% へ増えました。

### 非回帰

**火の魔女・氷術師は候補列（300 seed）とランタイムトレースが SHA-256 で完全一致**。
`save_version` は v6 のまま（draft へ pity カウンタを 1 キー加算的に追加しただけ）。
スキルの数値・進化条件・闘気 / コンボ / 回復 / 体勢は 1 件も変えていません。

詳細は `docs/warrior-evolution-guidance.md`、修正前の実測分析は `docs/warrior-draft-analysis-wave1.md`、
実ブラウザの確認手順は `docs/test-guide.md` の Milestone 8-C.1 節。
...
**Milestone 8-F の検証**: 戦士の**完成監査**（**新 active / passive / 進化 / ジョブは 1 件も追加していない**）。
M8-E で戦士が **active30 / passive4 / evolution18 = 52** に到達し、火の魔女（M8-A）・氷術師（M7-E）と
同規模になったのを受けた総点検です。カタログ整合性（30 / 4 / 18 / 52・JSON だけ / class だけ / docs だけ = 0・orphan 0）・
3 ジョブのプール分離（互いに素・他ジョブ混入 0）・rarity と役割（common7 / uncommon13 / rare10・legendary 段は進化 18 件が担う）・
全 18 進化の到達可能性・production の `SkillDraftManager` ＋ `SeededRandom` ＋ 実 rarity / 実 guidance / 実 pity /
実 synergy / 実 reroll・banish・skip による抽選シミュレーション（**200 seed × 5 戦略 ＋ 素朴戦略 × active枠 4 / 6 / 8**・
`Math.random` を 1 度も使わない）・`SkillAudit` 48 件・`recordCast` の 1:1 48 件・cooldown 保存 48 件・
`runtimeState` 48 件・固有機構（闘気 / コンボ / 撃破回復 / 不屈 / 軽減 / 反撃と弾き返しの調停 / 処刑 /
ノックバック・打ち上げ・掴み・投げ / エリートの体勢 / ボスの崩し / 移動 / 投擲と陣）・Job Lv1〜100 と passive 4 種・
telemetry と F8 / F9（F10 非回帰）・性能（`PoolManager` / `SpatialGrid` / `skillCaps`）・セーブ往復と旧セーブ・
決定性・バランス分布を、`node tests/warrior-completion-catalog.mjs` ほか**新規23スイート**で検証しました（`HEAVY=1` で seed / 時間増）。

他ジョブ混入・不正候補・重複候補・slot違反・不正進化は**すべて 0 件**、
提示 0 / 取得 0 の active・passive・進化も **0 件**、到達不能な進化も **0 件**です。
進化到達率は **M8-C.1 のしきい値を 1 つも下げずに**満たしています
（枠4: 6 戦略すべてが ≥1 個 98.0〜100% / 平均 2.19〜3.55 / 0 個 ≤2.0%、
枠6: ≥1 個 100% / ≥2 個 96.5〜100% / 平均 3.37〜5.33、
枠8: ≥1 個 99.5〜100% / ≥2 個 96.5〜100% / 平均 4.51〜7.05）。
最頻進化のシェアは **10.7%**（目標 35% 以下）、build の種類は 枠4 185〜199 / 200・枠6 198〜200 / 200・枠8 199〜200 / 200 で、
**候補ゼロはすべて飽和由来**（所持がすべて上限＋残り進化なし。飽和以外は 0 件）です。

監査で見つかった不備 **8 件** と data の死にフィールド **1 件**を修正しました
（優先度は M8-F の指定順＝crash → save 破損 → プール漏れ → … → balance）。
**(1) 戦士 48 スキルの `restoreState({cdLeft})` が負数 / NaN / ±Infinity / 桁外れをそのまま採用していた**
（改ざん保存で `_cd = NaN` にすると、以後そのスキルが二度と撃てない / 常に撃てる状態を作れた）→
`WarriorSkillBase` / `WarriorEvolvedBase` へ共通の `restoreCd()` を追加し、非有限値は採用せず有限値を ±120s へクランプ、
35 ファイルの生の代入をこの 1 か所へ寄せました（**火 / 氷の restore は 1 行も変えていません**）。
**(2) 陣（`placeRallyField`）の半径が上限クランプされていなかった** → `balance.warrior.rally.maxRadius`(280) を追加。
**(3) 反撃窓（`beginCounterWindow` と復元）の回数と持続が上限クランプされていなかった** →
`counter.maxCountersPerWindow`(6) / `counter.maxWindowMs`(6000) を追加し、開始時と復元時の両方でクランプ。
**(4) 旋風斬の回転残り時間が復元でクランプされず、永久に回り続ける状態を作れた** → data の `duration` で頭打ちに。
**(5) 進化済みの基礎 active が空き枠へ「新規」候補として戻っていた**（200 seed 中 38 回。取得すると進化と基礎を
同時所持でき「元 active と同時稼働しない」に違反した）→ 抽選コンテキストへ `evolvedBaseIds` を**加算的に**追加し、
`SkillDraftManager._eligible` が除外します（未指定なら従来と完全に同一挙動なので**火 / 氷の候補列ハッシュは不変**）。
**(6) M8-B / M8-C の 21 スキルが `destroy()` 後も発動し続けた**（進化置換・Scene 終了のあとに攻撃が飛ぶ余地）→
2 つの戦士基底の `update()` に破棄ガードを、`destroy()` に `_dead = true` を 1 か所だけ置きました。
**(7) 決闘マーカー（`Enemy._duelMark`）が時間切れ / 再指定 / 解除で外れず、生きた敵に残留していた** →
`BattleScene._setDuelMark` / `_clearDuelMark` で寿命を一元化し、決闘が終わったフレームと Scene 終了で必ず外します。
**(8) `crimson_execution`（血断処刑）が base より弱かった**（群れの中で総ダメージ 69% の逆転）→
`safetyCaps.maxTargetsPerStrike` を **10 → 20**（修正後 evo/base = 1.37・依然として有界で品質上限 24 以下）。
死にフィールド `charge_slash.config.hitOncePerTarget` は実装から読むようにしました
（現在の data は `true` なので挙動は不変）。

**敵 HP / 攻撃力 / 経験値 / スキルの damage・cooldown / 難易度倍率 / 報酬は 1 件も変えていません。**
変更した data は上の `safetyCaps` 1 値と、上限クランプ用のキー 3 件（`rally.maxRadius` /
`counter.maxCountersPerWindow` / `counter.maxWindowMs`。現在の data では上限に達しないため正常プレイの挙動は不変）だけです。
**全185スイート通過・`validate-data` 0エラー0警告**、`save_version` は **v6 のまま**（保存キーを 1 つも増やしていません）。
火の魔女・氷術師の data・実装は候補列・ランタイム・セーブ・状態異常 RNG とも 1 件も変えておらず、
**変更前後で SHA-256 完全一致**です（`tests/three-job-completion-nonregression.mjs`）。
**実際の描画・当たり判定・体感バランス・60FPS 維持は Phaser 依存のためヘッドレスでは未計測**です。
GitHub Pages を実ブラウザ（`?debug=1` の F8/F9/F10）で開き、`docs/test-guide.md` の
