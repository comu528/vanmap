...
> ⚠️ **開発状況**: 現在 **Milestone 8-D**（戦士スキル拡張 Wave2）まで実装済みです。
> M8-D では戦士の **active を 10 種追加して計 25 種**、**進化を 5 種追加して計 13 種**にしました
> （passive は 4 種のまま・Job Lv1〜100 も据え置き）。
> 追加した active は 昇竜斬 / 鉄壁突進 / 燕返し / 豪腕投げ / 三段砕き / 刃防陣 / 狂戦猛進 /
> 戦斧投擲 / 破城膝撃 / 戦旗招集、進化は 天衝断空 / 城塞蹂躙 / 無影燕返 / 山岳投擲 / 血盟戦旗 です。
> 打ち上げ・前面防御・掴み / 投げ・戦旗の陣・低 HP スケーリングという新しい機構を足しましたが、
> **いずれも新しい状態異常を作らず**、`WarriorCombatSystem` の中で `balance.json` の上限つきで動きます。
> **打ち上げと掴みが効くのは通常敵だけ**（エリートは体勢削り / その場叩きつけ、ボスは重い一撃へ置換）。
> 前面防御は**方向の分かる被弾だけ**を受け流し、方向なし（DoT / 全体攻撃）には一切効きません。
> **Job Lv80「打撃数 +1」の対象は 3 ジョブとも 6 種のまま**で、遠距離へ飛ぶ弾も 1 つも増えていません。
> active が 25 に増えたぶんの抽選の薄まりは、**M8-C.1 のしきい値を 1 つも下げずに**
> guidance へ data キーを 1 つ足すだけで吸収しました（`activeSupportWeightMultiplier`）。
> **火の魔女・氷術師は候補列 300 seed と 48 スキルのランタイムとも SHA-256 で完全一致**・
> **save_version は v6 のまま**。実ブラウザでの確認は未実施。
> 詳細は `docs/warrior-wave2.md`・`docs/warrior-evolution-guidance.md`・`docs/skill-catalog.md`。
>
