...
## Milestone 8-C.1: 進化導線の直し方（避けた設計）

M8-C で active が 5 → 15 になり、枠 4 の進化到達率が 92.5% → 46.5% へ落ちた。
**しきい値を下げて結果に合わせるのではなく、production の導線を直した。**

### 採った設計

「どの進化元を取るか」は**レアリティ階層に委ねたまま**、
「取った進化元を最後まで伸ばす」側だけを助ける。data の進化レシピからのみ対象を導出する。

### 避けた設計

| 避けたもの | 理由 |
|-----------|------|
| テスト閾値を下げる | 問題を隠すだけ。仕様でも禁止 |
| 進化候補の強制挿入 / 条件未成立での提示 | 進化条件が意味を失う |
| レベルアップ候補を進化 1 択へ固定 | build の自由が消える。毎回同じ周回になる |
| 特定 skill ID のハードコード | data を足すたびに実装を触ることになる。特定進化への集中も起きる |
| 進化元の**新規取得**を無条件に優遇 | 実測でレアリティ階層が歪んだ（rare の進化元の取得率が同 rarity 中央値の 4.8 倍）。補助が揃っているときだけに限定した |
| 無関係な新 active の重みを抑制 | 実測で到達率がほとんど変わらず、build 多様性だけ下がったので採用しなかった |
| global な synergy / rarity の変更 | 火 / 氷の非回帰を壊す |
| 候補生成後の後付け差し替え | 決定論と RNG 消費を壊す |
| pity を保存しない | save → reload でリセットして稼げてしまう |
| 進化元の Lv 要件・補助の Lv 要件を下げる | バランス変更にあたる。M8-C.1 の範囲外 |

### 戦士だけに補助が要る理由

火 / 氷は進化の補助が **active 中心**なので、補助を伸ばす過程で基礎も育つ。
戦士は 8 進化中 7 件が **passive 補助**で、passive は枠 4・maxLevel 4〜5 と軽く早期に埋まる。
...
戦士のカタログは **active 30 / passive 4 / evolution 18** まで拡張された（M8-E で完了）。
これで火の魔女・氷術師と同規模になった。
...
## Milestone 8-E: 最終カタログ（active 30 / passive 4 / evolution 18）
...
### 避けた設計（M8-E で改めて守ったこと）

| 避けたこと | 代わりにどうしたか |
|-----------|-------------------|
| 画面端まで届く斬撃波 | 貫穿突きの射程は `line.maxLineLength`（300px）で頭打ち。**弾も出さない** |
| 遠距離だけで完結する戦士 | 直線も進軍も**踏み込み / 前進**が前提。射程を伸ばす手段は増やしていない |
| 全スキル円形 AoE | 直線（貫穿突き）・単体寄り（一騎討ち）・移動制圧（震天踏破）で形を散らした |
| 新しい状態異常 | 決闘も構えも `WarriorCombatSystem` 上の状態。相手には何も貼らない |
| ボスを止める / ロックする手段 | 決闘は自分のバフだけ。延長にも合計上限があり永久ロックできない |
| 完全な弾無効化 | 弾けるのは通常の敵弾だけ・窓ごとに上限つき・予兆 / 光条 / 地形 / DoT は弾けない |
| 無限反射 | 反射弾は世代 1 で止まる。同じ弾も 2 度弾かない |
| 自傷して稼ぐ構成 | 構えの対価は軽減の小幅低下だけ。自傷もライフスティールもしない |
