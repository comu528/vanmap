...
- **実機 FPS / 長時間のメモリ / 高負荷ピーク / 通常 draft の到達点**は M9-A.2 の
  実ブラウザ長時間プレイテストで実測した（`./browser-longrun-results.md`・
  最終判定は `./final-balance-verdict.md`）。本書の Node 実測はいずれも固定 build なので、
  通常 draft の到達点とは**別の軸**として読むこと。
- **人間の体感（楽しいか・爽快か・難しすぎないか）は引き続き未確認**
  （`./human-playtest-gate.md` の open な manual gate）。Node でも自動ブラウザ操作でも判定しない。
