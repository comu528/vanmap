...
| **戦士** | **15** | **8** | physical | 接近・コンボ・体勢崩し（＋ M8-C の処刑 / 反撃 / 引き寄せ） |

戦士のカタログは火 / 氷の半分。Wave2 以降で 30 / 18 へ揃える予定
（`TODO.md` の「次のマイルストーン候補」）。

詳細な設計意図と「避けた設計」は `./warrior-design.md` と `./warrior-wave1.md`。
