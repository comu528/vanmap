...
## Milestone 9-A: 品質は「見た目の設定」になった（遊びの設計への影響）

M9-A は監査だけの回で、新しいコンテンツもバランス変更も無い。遊びの意味は 1 つ:

**エフェクト品質はもう戦闘結果に影響しない。**

- これまで: low は敵が最大 60 体（high は 200・ultra は 320）・1 撃の対象数も少ない・
  魂炎の一部強化が無効・ヒットストップ無し —— **低品質は実質的に別バランス**だった。
- これから: どの品質でも敵の数・対象数・弾数・状態付与・XP・kill・Combo / Fury は**完全に同じ**。
  品質が変えるのは粒子・軌跡・破片・画面シェイク・閃光・ダメージ数字の表示だけ。
- 基準（canonical）は**出荷既定の high**。low / medium のプレイヤーは gameplay が high 相当へ
  強化され、ultra は演出だけ最大のまま gameplay が計測実績のある high へ揃う。

敵 HP / 攻撃力 / 経験値 / 報酬 / 難易度 / スキル数値は 1 件も変えていない。
詳細は `./quality-gameplay-invariance.md`。
