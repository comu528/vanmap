...
## Milestone 8-D: 戦士 Wave2 のバランス検証

### F8（バランス分析）へ追加した戦士 Wave2 の項目

```
— 戦士 Wave2（M8-D 実動作カウンタ）—
打ち上げ N回（拒否 N 体勢変換 N）※通常敵のみ
前面防御 稼働 Ns 受け流しN 側面N 背面N
掴み N回（拒否N 中断N） 投げ N回 Npx
三段 N段 刃防陣 Ntick 稼働 Ns
低HP倍率 平均×1.xxx 戦斧 行きN/帰りN 踏み込み N回 Npx
戦旗 N本 稼働Ns 内側Ns 血の誓いN回
active補助の進化 n/m（heaven_crushing_descent mountain_hurl）
```
...
| 警告 | 意味 |
|------|------|
| ⚠ 昇竜斬はあるが打ち上げ0 | 打ち上げ経路が動いていない |
| ⚠ 前面防御は稼働しているが受け流し0 | 被弾方向が渡っていない |
| ⚠ 豪腕投げはあるが掴み0 | 掴み可否か射程の問題 |
| ⚠ 戦旗は立つが内側に居た時間0 | 陣の中心がずれている |
| ⚠ 戦斧の帰りが 1 度も当たっていない | 折り返し判定の問題 |
| ⚠ build が 1 進化へ偏っている | 抽選が 1 択に寄っている |

### 抽選シミュレーション

```
node tests/warrior-wave2-draft.mjs      # 25 active / 13 進化の網羅・集中度
node tests/warrior-wave2-guidance.mjs   # guidance の追調整（過剰誘導なし）
HEAVY=1 node tests/warrior-wave2-draft.mjs   # seed 数を 500 まで増やす
```
...
|------|------|---------------------------|
| 素朴戦略 枠4/40lv・進化 1 個以上 / 平均 / 0 個 | 85.0% / 1.17 / 15.0% | 80% 以上 / 1.0 以上 / 20% 以下 |
| 素朴戦略 枠6/60lv・1 個以上 / 2 個以上 / 平均 | 99.0% / 88.0% / 2.38 | 95% / 60% / 1.7 |
| 素朴戦略 枠8/80lv・1 個以上 / 2 個以上 / 平均 | 100.0% / 98.5% / 3.46 | 95% / 65% / 1.8 |
...
## Milestone 8-F: 戦士 完成監査のバランス検証

M8-F は**バランスを意図的に変えていない**回。検証の目的は数値の調整ではなく、
**構造的な異常（死にスキル / 進化の逆転 / 永久状態 / 一極集中 / 場外）が無いこと**の確認。

### 回すコマンド

```
node tests/warrior-completion-balance.mjs      # 分布・逆転・永久状態・拘束・弾き返し
node tests/warrior-completion-performance.mjs  # 4 品質 × 10 分相当・1 フレーム最大処理
...
### 測定条件（`warrior-completion-balance.mjs`）

敵 30 体 + エリート 6 体 + ボス 1 体（HP 1e9 で不死）、`active` 30 種すべてを Lv8 で同時所持、
`dt=32ms` × 9000 フレーム（約 4.8 分相当・`HEAVY=1` で 18000）、quality=high、
40 フレームごとに被弾、200 フレームごとに敵弾 10 発を追加、
HP は 250 フレームごとに満タン ⇄ 20% を往復（低 HP 補正・撃破回復・不屈をすべて経路に乗せる）。

### 実測（M8-F 時点）

| 指標 | 実測 | しきい値 |
|------|------|----------|
| 最大ダメージシェア | `great_cleave` **13.6%** | 35% 以下 |
| ダメージまたは utility を出さない active | **0 件** | 0 件 |
| evo/base < 0.9 の進化 | **0 件**（最小 1.07・最大 6.14） | 0 件 |
| 全部盛りでの進化シェア最大 | `thousand_blade_dance` **20.6%** | 45% 以下 |
| 永久状態（構え / 決闘 / 弾き窓 / 陣 / 解放 / 露出 / 掴み / 反撃窓） | **すべて 0 に戻る** | 0 |
| ボス崩し / 露出合計 / しきい値 | 39 回 / 118.9s / **×3.00** | 露出が連続しない |
| 弾き返しの無効化率 | 検知 202,414 / 弾き 390（**0.2%**） | 完全無効化にならない |
| 闘気解放 | 24 回 / 稼働率 **41.9%** | 常時稼働にならない |
| 最終Wave 5 種の合計シェア | **11.7%**（既存 25 種 88.3%） | 既存を全ハズレにしない |
| 場外へ出た敵 | **0 体** | 0 体 |
| 自傷経路 | **実装に存在しない**（`hp -=` / `selfDamage` / `hpCost` が無い） | 無い |

### 性能（`warrior-completion-performance.mjs`）
...
> これは **Node 純ロジックの計算時間**であり、**実ブラウザの FPS ではない**。
> 描画・入力・Phaser の内部処理は含まれていない。

品質で主発動回数が前後したのは `great_cleave` の 1 件だけ（58 → 59）。
低品質では 1 撃あたりの対象数が少なくコンボの伸びがわずかに遅く、
攻撃速度しきい値を越えるタイミングが 1 回分ずれる。
**1 発動あたりのダメージと規則の階層（`low ≤ medium ≤ high ≤ ultra`）は 4 品質で不変**。
M8-B からある 3 ジョブ共通の帰結で、M8-F で新たに生じた差ではない。

### M8-F で動かした唯一のバランス値

`crimson_execution.safetyCaps.maxTargetsPerStrike` を **10 → 20**
（evo/base = 0.69 の逆転を解消。修正後 1.37）。
巻き戻すと `warrior-completion-balance.mjs` の
