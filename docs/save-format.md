...
## Milestone 8-F: 戦士 完成監査（`save_version` は v6 のまま・**キーを 1 つも増やしていない**）

M8-F は**保存フォーマットを一切変えていない**。代わりに**復元の入口を固くした**回。
旧セーブはそのまま読める（移行処理なし）。
...
| 対象 | これまで | M8-F |
|------|----------|------|
| 戦士 48 スキルの `cdLeft` | `typeof === 'number'` なら**そのまま `_cd` へ代入**（負数 / NaN / ±Infinity / 1e18 も通った） | 共通 `restoreCd()` を通る。数値以外は採用せず、**非有限値も採用せず**、有限値は ±`MAX_RESTORED_CD_MS`(120s) へクランプ |
| 旋風斬 / 血戦旋風の `spinLeftMs` / `spinTickLeft` | そのまま採用（永久回転を作れた） | data の `duration` で頭打ち |
| 陣の `radius` | そのまま採用（半径 1e9 = 永久バフを作れた） | `balance.warrior.rally.maxRadius`(280) でクランプ |
| 反撃窓の回数 / 持続 | そのまま採用（999 回・1e9ms = 実質無限反撃を作れた） | `counter.maxCountersPerWindow`(6) / `counter.maxWindowMs`(6000) でクランプ（**開始時も復元時も**） |

### `restoreCd()` の仕様

```
restoreCd(value):
  value が number 型でなければ  → 採用しない（false を返す）  // 文字列 / null / undefined / object
  Number.isFinite(value) が偽   → 採用しない                  // NaN / ±Infinity
  それ以外                      → _cd = clamp(value, -120000, +120000)
```

- **クランプ下限が 0 ではなく −120s** なのは意図的。フレーム末に `_cd` が
  小さな負の値になる状態が正当に保存されるため、0 へ丸めると保存 → 復元の一致が壊れる。
- 採用しなかった場合、`_cd` は**コンストラクタの初期値のまま**（＝そのスキルの通常の初期 CD 状態）。
  例外を投げないので、壊れた保存でも起動不能にならない。
- **火の魔女・氷術師の 27 ファイルはこの経路を通らない**（従来の生の代入のまま）。
  火 / 氷の復元挙動は 1 バイトも変わっていない。

### 監査で確認した保存 / 復元の性質（48 スキル全件）

- `cdLeft` が **0 / 中間 / 最大 / 最大 −1** のいずれでも往復して一致する。
- 復元直後に**無料 cast が出ない**（CD が残っていれば撃たない）。
- 攻撃速度 / 修羅の構え / 闘気解放で CD 倍率が変わっても保存が壊れない。
- 進化置換の前後で CD が混ざらない。
- **オブジェクト参照 / Phaser の実体を保存しているスキルは 0 件**
  （敵を指す必要があるものは安定 runtime id `_seq` で保存する。例: `chain_hook` の `hookSeq`）。
- 旧セーブ（キー欠落 / `warriorState` なし / v5 以前）で例外にならない。
- 復元は**新規発動として数えない**ので telemetry（`duelStarts` / `tranceStarts` / `deflectWindows`）が
  水増しされない。
- `_` で始まる telemetry キーは**意図的に復元しない**（周回内の一時的な計測なので持ち越さない）。

### 保存しないもの（M8-F でも変えていない）

掴み（`_grab`）/ 弾いた弾の id 集合（`_deflectedIds`）/ 反射弾そのもの /
進行中の突き・進軍・投げ・段・斧の飛行 / cast 予算 / 同一敵の命中記録 /
敵側の決闘マーカー（`_duelMark`）。
